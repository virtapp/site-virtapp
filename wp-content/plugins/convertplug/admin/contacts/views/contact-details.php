      ...
      if( !empty( $contacts ) ){
        foreach( $contacts as $key => $list ){
          ...
          <div class="bend-heading-section bsf-connect-header bsf-connect-list-header <?php if( empty( $contacts ) ){ echo 'bsf-connect-empty-header'; } ?>">
            <h1><span class="cp-strip-text" style="max-width: 460px;top: 10px;" title="<?php if( !empty($name) ) { echo esc_attr( $name ); } else { echo __( 'Subscriber Details', 'smile' ); } ?>"><?php if( !empty($name) ) { echo esc_attr( $name ); } else { echo __( 'Subscriber Details', 'smile' ); } ?></span> <a class="add-new-h2" href="?page=contact-manager&view=contacts&list=<?php echo esc_attr( $_GET['list'] ); ?>"><?php _e('Back to Campaigns List', 'smile'); ?></a></h1>
