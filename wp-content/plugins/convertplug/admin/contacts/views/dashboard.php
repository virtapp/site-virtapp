...
/*    if ( count($uninstalled_addons) > 0 ) {
      ...
      $msg = "It seems you recently upgraded to the 2.0 version. Read the changelog <a target='_blank' href='https://www.convertplug.com/plus/category/changelog/'>here</a>. To see your previous campaigns (".ucfirst(implode(",",$uninstalled_addons))."), you will need to install the free addon from <a target='_blank' href='".admin_url('admin.php?page=bsf-extensions-14058953')."'>this</a> page.";
    ...
    if( is_array($smile_lists) ) {
	    foreach( $smile_lists as $key => $list ){
      $temp_contact = array();
			$provider = $temp_contact['provider'] = $list['list-provider'];
			$listName = $temp_contact['listName'] = str_replace(" ","_",strtolower( trim( $list['list-name'] ) ) );
			$list_id = $temp_contact['list_id'] = isset( $list['list'] ) ? $list['list'] : '';
			$mailer = $temp_contact['mailer'] = str_replace(" ","_",strtolower( trim( $provider ) ) );
 ...
 //var_dump($all_contacts);
    ...
    if ( isset( $_GET['orderby'] ) ) {
      ...
      switch( $orderby ) {
        case "list" :
          $sortingListClass  = 'sorting-'. $order;
        break;
        case "list-name" :
          $sortinglistNameClass = 'sorting-'. $order;
        break;
        case "list-provider" :
          $sortingProviderClass  = 'sorting-'. $order;
        break;
        case "contacts":
         	$sortingContactsClass = 'sorting-'. $order;
        break;
      }
    }
    ...
    if( isset( $_POST['sq'] ) && esc_attr( $_POST['sq'] ) == '' )
       $sq = '';
    ...
    $list_ids_arr = json_encode( $all_contacts ); 
...
<div class="wrap about-wrap bsf-connect bsf-connect-campaign bend">
  <div class="wrap-container">
   ...
   <form method="post" class="cp-export-contact" action="<?php echo esc_url($form_export_action); ?>">
      <input type="hidden" name="list_id" value='<?php echo $list_ids_arr; ?>' />
      <a class="action-list action-download-contact bsf-connect-download-csv" href="#" target="_top" style="margin-right: 25px !important;"><i style="line-height: 30px;" class="connects-icon-download"></i><span class="action-tooltip"><?php _e( "Export All contacts", "smile" ); ?></span></a>
    </form>
              ...
              <td scope="col" class="manage-column column-list">
              	...
              	if( isset( Smile_Framework::$addon_list[strtolower($provider)]['mailer_type'] ) ) {
                    ...
                    if( count( $provider_list_name ) > 0 && is_array( $provider_list_name ) ) {
                      ...
                      echo ( count( $str ) > 1 ) ? $first_tag . ' & ' . $tooltip_html : $first_tag;
