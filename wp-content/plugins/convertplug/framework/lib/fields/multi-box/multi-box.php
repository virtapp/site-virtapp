...
function smile_multi_box_scripts($hook){
	...
	if( $cp_page !== false || isset( $_GET['view'] )){
		if( ( isset( $data['cp-dev-mode'] ) && $data['cp-dev-mode'] == '1' ) && isset( $_GET['style-view'] ) && ( $_GET['style-view'] == "edit" || $_GET['style-view'] == "variant" ) ){
			wp_enqueue_style('multi-box',SMILE_FRAMEWORK_URI . '/lib/fields/multi-box/multi-box.css');
				...
				$output .= '<select class="cp_mb_select" name="input_type" id="mb-input_type-'.$uniq.'">';
					...
					if(!empty($input_types)) :
						foreach ($input_types as $key => $type) :
							...
							}else if($current_input_val === $type && ($type === 'checkbox')) {
								$is_checkbox = true;
							}
						endforeach;
					endif;
