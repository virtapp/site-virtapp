...
if( !function_exists( "slidein_generate_style_css" ) ) {
	function slidein_generate_style_css( $custom_css ) {
			/* CP - SlideIn Styling */
		echo '<style type="text/css" id="">'.$custom_css.'</style>';		
	}
}
...
if( !function_exists( "cp_slidein_global_before_init" ) ){
	function cp_slidein_global_before_init( $a ) {
		...
		$styleType                = $referrer_data = $autoclose_data = $bg_repeat = $bg_pos = $bg_size = $bg_setting = $close_img_class = $close_img = '';
		$timezone                 = $el_class = $slide_bg_style = $close_html = $slidein_size_style = $close_class = $windowcss = $font = '';
		$impression_disable_class = $scroll_data = $scroll_class = 	$inactive_data = $data_redirect = $overlay_effect = '';
		$slideshadow              = $slideradius = $placeholder_font = $hide_image = $load_on_count = $slide_position = '';
		$load_after_scroll        = $load_on_duration = $close_btn_on_duration = '';
		$form_data_onsubmit       = $optin_widgetclass = $cp_close_body = $inline_text = $toggleclass = $slide_toggle_class = $slide_in_bg_image = $customcss  = $windowcss = $inset = $css_style = '';
		...
		$toggle_btn_font_size    = $toggle_btn_border_radius = $toggle_button_border_color = $toggle_btn_padding_tb = '';
		...
		if( ( isset( $a['toggle_btn'] ) && $a['toggle_btn'] === '1' ) ) {
			$side_btn_style .=  '.slidein-overlay.content-'.$uid.' .'. $a['side_btn_style'] . '.cp-slide-edit-btn {
									font-family: ' . $font . ';
		...
		$si_onload = ( isset( $a['manual'] ) && $a['manual'] === 'true' )? '' : ' si-onload cp-global-load ';
		...
		$slide_content_data = $cp_container_class = $cp_overlay_class = $slide_overlay_data  = $slide_overlay_class = '';
		...
		$slide_overlay_data .= $isScheduled.' ';
		...
		//$slide_content_data .= $cp_slidein_visibility.' ';
		$slide_content_data .= $alwaysVisible.' ';
...
if( !function_exists( "cp_slidein_global_after_init" ) ){
	function cp_slidein_global_after_init( $a ) {
		...
		$close_html = $el_class = $close_class = '';
