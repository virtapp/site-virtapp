...
if( !function_exists( "modal_theme_jugaad" ) ) {
	function modal_theme_jugaad( $atts, $content = null ){
		...
		if( !$old_form_user && $is_gradient_form == 'gradient' && $form_type_set && $a['style'] == 'jugaad'){			
			$form_overaly_css = generateBackGradient($form_bg_gradient);
		}else{
			$form_overaly_css = cp_add_css('background-color', $form_bg_color );
		}

		if( $form_bg_image !== '' && $is_gradient_form == 'image' ){
		...
		if( !$old_content_user && $is_gradient_content == 'gradient' && $form_type_set && $a['style'] == 'jugaad'){
			$content_overaly_css= generateBackGradient( $content_bg_gradient );
		}else{
			$content_overaly_css = cp_add_css( 'background-color', $content_bg_color );
		}
	
		if( $content_bg_image !== '' && $is_gradient_content == 'image' ){
