...
( function() {
	...
	FusionPageBuilder.ElementSettingsView = window.wp.Backbone.View.extend( {
		...
		optionChange: function( event ) {
			...
			if ( ! jQuery( event.target ).hasClass( 'fusion-skip-debounce' ) && ( this.model.get( 'noTemplate' ) || jQuery( event.target ).hasClass( 'fusion-debounce-change' ) || ajaxDynamicParam ) ) {
				if ( !this.debouncedOptionChanges[ paramName ] ) {
					this.debouncedOptionChanges[ paramName ] = _.debounce( _.bind( this.optionChanged, this ), 500 );
				}
		...
		getParamName: function( $target, $option ) {
			...
			// Non single dimension fields.
			if ( $target.closest( '.fusion-builder-option.dimension' ).length && ! $target.closest( '.single-builder-dimension' ).length ) {
		...
		checkPageTemplate: function() {
			if ( 'fusion_builder_container' === this.model.get( 'element_type' ) ) {
				if ( '100-width.php' !== FusionApp.data.postMeta._wp_page_template ) {
					this.$el.find( 'li[data-option-id="hundred_percent"]' ).hide();
				} else {
					this.$el.find( 'li[data-option-id="hundred_percent"]' ).show();
				}
			}
		},
