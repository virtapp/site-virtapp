		...
		FusionPageBuilder.BaseColumnView = FusionPageBuilder.BaseView.extend( {
			...
			getTemplateAtts: function() {
				...
				if ( 'fusion_builder_column' === this.model.attributes.element_type ) {
					styleSelector = '.fusion-builder-column-' + this.model.get( 'cid' );
				} else {
					styleSelector = { regular: '.fusion-builder-column .fusion-column-wrapper-' + this.model.get( 'cid' ), hover: '.fusion-builder-column:hover .fusion-column-wrapper-' + this.model.get( 'cid' ) };
				}
			...
			sizesShow: function( event ) {
				...
				if ( ! this.$el.hasClass( 'active' ) ) {
					...
					if ( 54 > columnOffsetTop - 121 || conditional ) {
