	...
	if ( ! class_exists( 'FusionSC_FusionGallery' ) ) {
		...
		class FusionSC_FusionGallery extends Fusion_Element {
			...
			public function get_image_markup( $image_id, $image = '', $parent_args ) {
				...
				$this->image_data = $fusion_library->images->get_attachment_data_by_helper( $image_id . '|' . $parent_args['picture_size'], $image );
				...
				if ( 'masonry' === $parent_args['layout'] && $this->image_data ) {
					...
					$element_orientation_class                = $fusion_library->images->get_element_orientation_class( $this->image_data['id'], [], $parent_args['gallery_masonry_grid_ratio'], $parent_args['gallery_masonry_width_double'] );
				...
				$image = $fusion_library->images->apply_lazy_loading( $image, null, $image_id, $this->parent_args['picture_size'] );
				...
				if ( isset( $_POST['children'] ) && isset( $_POST['gallery'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification
					...
					if ( is_array( $image_ids ) && ! empty( $image_ids ) ) {
						foreach ( $image_ids as $index => $image ) {
							...
							$image_data = fusion_library()->images->get_attachment_data( $image_id, $image_size );
							$image_url  = '';
							$pic_link   = '';
							...
							// Get the correct image orientation class.
							$element_orientation_class          = fusion_library()->images->get_element_orientation_class( $image_data['id'], $post_thumbnail_attachment, $gallery_settings['gallery_masonry_grid_ratio'], $gallery_settings['gallery_masonry_width_double'] );
							$element_base_padding               = fusion_library()->images->get_element_base_padding( $element_orientation_class );
