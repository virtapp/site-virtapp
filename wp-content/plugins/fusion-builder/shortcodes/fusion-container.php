...
if ( ! class_exists( 'FusionSC_Container' ) ) {
	...
	class FusionSC_Container extends Fusion_Element {
		...
		public function render( $atts, $content = '' ) {
			...
			$this->args = FusionBuilder::set_shortcode_defaults( self::get_element_defaults(), $atts, 'fusion_builder_container' );
			extract( $this->args );
			...
			$overlay_style   = '';
			$lazy_load_style = '';
			$classes         = 'fusion-fullwidth fullwidth-box fusion-builder-row-' . $this->container_counter;
			$outer_html      = '';
			...
			if ( $is_gradient_color ) {
				$lazy_load_style .= 'data-bg-gradient="' . Fusion_Builder_Element_Helper::get_gradient_string( $this->args ) . '"';
			}
			...
			if ( $video_bg ) {
				...
				// Video Overlay.
				if ( $is_gradient_color ) {
					$overlay_style .= 'background-image: ' . Fusion_Builder_Element_Helper::get_gradient_string( $this->args ) . ';';
				}
			...
			if ( $is_gradient_color ) {
				$style .= 'background-image:' . Fusion_Builder_Element_Helper::get_gradient_string( $this->args, 'main_bg' );
			}
			...
			if ( 'yes' === $fade && ! empty( $background_image ) && false === $video_bg ) {
				...
				if ( $is_gradient_color ) {
					$fade_style .= 'background-image: ' . Fusion_Builder_Element_Helper::get_gradient_string( $this->args, 'fade' );
				}
			...
			if ( '100%' === $fusion_settings->get( 'site_width' ) ||
				...
				( '1' == FusionBuilder::get_page_option( 'portfolio_width_100', 'portfolio_width_100', $c_page_id ) || 'yes' === FusionBuilder::get_page_option( 'portfolio_width_100', 'portfolio_width_100', $c_page_id ) && 'avada_portfolio' === get_post_type( $c_page_id ) ) || // phpcs:ignore WordPress.PHP.StrictComparisons.LooseComparison
			...
			// Add filter styles.
			$filter_style = Fusion_Builder_Element_Helper::get_filter_style_element( $this->args, '.fusion-builder-row-' . $this->container_counter );
			if ( '' !== $filter_style ) {
				$output .= $filter_style;
			}
			...
			[
				...
				'params'            => [
					...
					'fusion_filter_placeholder'   => [
						'selector_base' => 'fusion-builder-row-',
					],
				],
			]
		)
