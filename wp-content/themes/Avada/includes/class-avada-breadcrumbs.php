...
class Avada_Breadcrumbs {
	...
	public function get_breadcrumbs() {
		...
		$this->wrap_breadcrumbs( $use_schema );
	 ...
	 *
	 * @access private
	 * @param bool $use_schema Decides whether schema markup should be used.
	 * @return void
	 */
	private function wrap_breadcrumbs( $use_schema = true ) {
		$schema = '';
		...
		$this->html_markup = '<div class="fusion-breadcrumbs"' . $schema . '>' . $this->html_markup . '</div>';
	}
	...
	private function get_breadcrumb_leaf_markup( $object_type = '' ) {
		...
		return $this->get_single_breadcrumb_markup( $title, '', false, true, true );
	...
	private function get_single_breadcrumb_markup( $title, $link = '', $separator = true, $microdata = true, $is_leaf = false ) {
		...
		$breadcrumb_content .= '<meta itemprop="position" content="' . $this->item_position . '">';
		$this->item_position++;
