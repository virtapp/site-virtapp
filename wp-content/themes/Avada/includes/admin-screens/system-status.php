...
<div class="wrap about-wrap avada-wrap">
	...
	<div class="avada-system-status">
		...
		<table class="widefat" cellspacing="0">
			...
			<tbody>
				...
				<tr>
					...
					$previous_versions_array = is_array( $previous_version ) ? $previous_version : [];
					...
					?>
					<td>
						<?php echo $previous_version; // phpcs:ignore WordPress.Security.EscapeOutput ?>
					</td>
				</tr>
			</tbody>
		</table>
		...
		?>
		<?php // Display Avada conversions if available. ?>
					...
					if ( ! empty( $plugin_data['Name'] ) ) {
						...
						<tr>
							...
							<td>
								...
								<?php printf( esc_attr__( 'by %s', 'Avada' ), '<a href="' . esc_url( $plugin_data['AuthorURI'] ) . '" target="_blank">' . esc_html( $plugin_data['AuthorName'] ) . '</a>' ) . ' &ndash; ' . esc_html( $plugin_data['Version'] ) . $version_string . $network_string; ?>
							</td>
						</tr>
						<?php
					}
