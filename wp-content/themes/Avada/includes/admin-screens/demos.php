...
<div class="wrap about-wrap avada-wrap">
	...
	<?php if ( Avada()->registration->is_registered() ) : ?>
		...
		asort( $all_tags, SORT_NATURAL | SORT_FLAG_CASE );

		$all_tags = array_merge( [ 'all' => esc_attr__( 'All Demos', 'Avada' ) ], $all_tags );
		...
		foreach ( $plugin_dependencies as $key => $plugin ) {
			$plugin_dependencies[ $key ]['active']    = fusion_is_plugin_activated( $plugin['file_path'] );
		 ...
		 */
		?>
		<div class="avada-importer-tags-selector" style="margin-bottom: 1.5em; <?php echo ( isset( $_GET['beta'] ) ) ? '' : 'display: none;'; // phpcs:ignore WordPress.Security.NonceVerification ?>">
			<select>	
