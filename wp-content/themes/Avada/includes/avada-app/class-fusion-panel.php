...
class Fusion_Panel {
	...
	public function get_page_options() {
		...
		foreach ( $ids as $id ) {
			if ( 'pyre_sidebar_1' === $id ) {
				if ( isset( $selected_sidebar_replacement[0] ) ) {
					$values[ $id ] = $selected_sidebar_replacement[0];
				} else {
					$values['sbg_selected_sidebar_replacement'] = [ 'default_sidebar' ];
				}
			} elseif ( 'pyre_sidebar_2' === $id ) {
				if ( isset( $selected_sidebar_2_replacement[0] ) ) {
					$values[ $id ] = $selected_sidebar_2_replacement[0];
				} else {
					$values['sbg_selected_sidebar_2_replacement'] = [ 'default_sidebar' ];
				}
