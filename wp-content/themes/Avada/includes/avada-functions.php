...
if ( ! function_exists( 'avada_ajax_avada_slider_preview' ) ) {
	...
	function avada_ajax_avada_slider_preview() {
		...
		$slider_type = ( isset( $_POST['data'] ) && isset( $_POST['data']['slidertype'] ) ) ? sanitize_text_field( wp_unslash( $_POST['data']['slidertype'] ) ) : get_post_meta( $post->ID, 'pyre_slider_type', true ); // phpcs:ignore WordPress.Security.NonceVerification
		$slider_demo = ( isset( $_POST['data'] ) && isset( $_POST['data']['demoslider'] ) ) ? sanitize_text_field( wp_unslash( $_POST['data']['demoslider'] ) ) : get_post_meta( $post->ID, 'pyre_demo_slider', true ); // phpcs:ignore WordPress.Security.NonceVerification
		...
		if ( 'layer' === $slider_type ) {
			$slider             = ( isset( $_POST['data'] ) && isset( $_POST['data']['layerslider'] ) ) ? sanitize_text_field( wp_unslash( $_POST['data']['layerslider'] ) ) : get_post_meta( $post->ID, 'pyre_slider', true ); // // phpcs:ignore WordPress.Security.NonceVerification
			...
			$slider             = ( isset( $_POST['data'] ) && isset( $_POST['data']['revslider'] ) ) ? sanitize_text_field( wp_unslash( $_POST['data']['revslider'] ) ) : get_post_meta( $post->ID, 'pyre_revslider', true ); // // phpcs:ignore WordPress.Security.NonceVerification
			...
			$slider             = ( isset( $_POST['data'] ) && isset( $_POST['data']['wooslider'] ) ) ? sanitize_text_field( wp_unslash( $_POST['data']['wooslider'] ) ) : get_post_meta( $post->ID, 'pyre_wooslider', true ); // // phpcs:ignore WordPress.Security.NonceVerification
			...
			$slider             = ( isset( $_POST['data'] ) && isset( $_POST['data']['elasticslider'] ) ) ? sanitize_text_field( wp_unslash( $_POST['data']['elasticslider'] ) ) : get_post_meta( $post->ID, 'pyre_elasticslider', true ); // // phpcs:ignore WordPress.Security.NonceVerification
...
if ( ! function_exists( 'avada_migrate_header_bg_postmeta' ) ) {
	/**
	 ...
	 * @return void
	 */
	function avada_migrate_header_bg_postmeta() {
		global $post;

		// Early exit if we don't have a post-ID.
		if ( ! is_object( $post ) || ! isset( $post->ID ) || ! $post->ID ) {
			return;
		}

		// Check if the header-background-color setting has already been migrated.
		if ( ! get_post_meta( $post->ID, 'pyre_combined_header_bg_color', true ) ) {
			...
			if ( $header_bg_po || is_numeric( $header_alpha_po ) ) {
				...
				// Reset dynamic-css caches for this post.
				Fusion_Dynamic_CSS::get_instance()->reset_post_transient( $post->ID );
				Fusion_Dynamic_CSS::get_instance()->post_update_option( $post->ID );
			}
		}
	}
}
