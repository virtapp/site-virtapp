...
class Fusion_FusionRedux {
	...
	public function get_field( $field, $section_id = null ) {
		...
		if ( $_GET && isset( $_GET['disable_dependencies'] ) ) { // phpcs:ignore WordPress.Security
			if ( $_GET['disable_dependencies'] === $args['id'] ) { // phpcs:ignore WordPress.Security
				$args['required'] = [];
			}
			if ( ! empty( $args['required'] ) ) {
				foreach ( $args['required'] as $key => $requirement ) {
					if ( isset( $requirement['setting'] ) && $_GET['disable_dependencies'] === $requirement['setting'] ) { // phpcs:ignore WordPress.Security
