...
class Avada_Scripts {
	...
	public function __construct() {
		...
		if ( ! is_admin() && ! in_array( $GLOBALS['pagenow'], [ 'wp-login.php', 'wp-register.php' ] ) ) {
			...
			add_action( $action, [ $this, 'wp_enqueue_styles' ] );
	...
	protected function enqueue_scripts() {
		...
		if ( fusion_get_option( 'live_search' ) || $is_builder ) {
			$scripts[] = [
				'avada-live-search',
		...
		if ( ! get_the_ID() || avada_is_page_title_bar_active( get_the_ID() ) || $is_builder ) {
			...
			Fusion_Dynamic_CSS::enqueue_style( Avada::$template_dir_path . '/assets/css/dynamic/page-title-bar.min.css', Avada::$template_dir_url . '/assets/css/dynamic/page-title-bar.min.css' );
		}
		...
		if ( ( Avada()->settings->get( 'use_animate_css' ) && ! class_exists( 'FusionBuilder' ) ) || $is_builder ) {
	...
	protected function localize_scripts() {
		...
		$shop_page_bg_color = fusion_get_option( 'content_bg_color', 'pyre_wide_page_bg_color', get_option( 'woocommerce_shop_page_id' ) );
		...
		$scripts = [
			...
			[
				...
				'avadaLiveSearchVars',
				[
					'live_search'       => fusion_get_option( 'live_search' ),
					'ajaxurl'           => admin_url( 'admin-ajax.php' ),
